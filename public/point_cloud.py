...
def main():
    ...
    for file in files:
        if file[-4:] == ".tif":
            ...
            img = cv2.resize(img, (500, 500))  # change here for more or less resolution
            images.append(img) 
    print(np.size(images))
    #print(images)

    # keep a blank mask
    #Array of zeros with the same shape and type as images[0]
    ...
    for index in range(1,len(masks)-1):
        ...
        #print(prev_mask[prev])
        prev_mask[prev] = curr[prev]
        addPoints(prev_mask, points, depth)

        # # do a slice on after
        next_mask = np.zeros_like(curr)
        next_mask[after] = curr[after]
    ...
    createPointCloud("test4.ply", points)
